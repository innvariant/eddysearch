...
class GeneticSearch(PopulationSearch):
    ...
    def step(self):
        ...
        population_eval = {
            member: self._eval_gene(member) for member in self._population
        }
        ...
        while len(self._population) < self._population_size:
            ...
            if operation == "random":
                ...
                if new_random_member is None:
                    raise ValueError(
                        "sample_gene() returned None but should return a randomly sampled gene for the population"
                    )
                ...
                if new_member is None:
                    raise ValueError(
                        "mutate(member) returned None but should return a randomly sampled gene for the population"
                    )
    ...
    def __str__(self):
        return "GeneticSearch(dim=%s, pop_size=%s, num_gens=%s)" % (
            self._dimensions,
...
class GeneticGridSearch(GeneticSearch):
    def __init__(self, binary_space=5, **kwargs):
        ...
        self._allele_codes = np.array(
            [
                2 ** (i * binary_space) - 1 - (2 ** ((i - 1) * binary_space) - 1)
                for i in range(1, self._num_alleles + 1)
            ]
        )
    ...
    def encode(self, x):
        ...
        return sum(
            self._restrict(alelle) * (2 ** (pos * self._binary_space))
            for pos, alelle in enumerate(x)
        )

    def decode(self, gene):
        return np.array(
            [
                (gene & code) >> (pos * self._binary_space)
                for pos, code in enumerate(self._allele_codes)
            ]
        )
    ...
    def crossover(self, gene1, gene2):
        ...
        return self.encode(
            np.array(
                [
                    np.random.randint(*self._shifted_interval(x1, x2))
                    for x1, x2 in zip(alleles1, alleles2)
                ]
            )
        )
    ...
    def mutate(self, gene):
        ...
        gene_alleles += np.random.randint(
            [-self._mutation_max] * len(gene_alleles),
            [self._mutation_max] * len(gene_alleles),
        )
    ...
    def __str__(self):
        return "GeneticGridSearch(dim=%s, pop_size=%s, num_gens=%s, bin_space=%s)" % (
            self._dimensions,
            self._population_size,
            self._num_generations,
            self._binary_space,
...
class GeneticRingSearch(GeneticSearch):
    ...
    def step(self):
        ...
        for p1, p2 in itertools.product(sorted_population[-k:], sorted_population[-k:]):
            cross_gene = self._crossover(
                p1, population_eval[p1], p2, population_eval[p2]
            )
            self._population.add(cross_gene)
        ...
        while len(self._population) < self._population_size:
            ...
            if operation == "random":
                ...
                mutation = np.random.uniform(
                    [-self._mutation_max_pos] * self._dimensions
                    + [-self._mutation_max_radius],
                    [self._mutation_max_pos] * self._dimensions
                    + [self._mutation_max_radius],
                )
    ...
    def __str__(self):
        return (
            "GeneticRingSearch(dim=%s, pop_size=%s, num_gens=%s, min_radius=%s, max_radius=%s)"
            % (
                self._dimensions,
                self._population_size,
                self._num_generations,
