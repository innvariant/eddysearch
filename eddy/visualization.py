...
def old_visualize_path(search_path, objective: Objective = None, space: int=None, infer_objective: bool=True, axis=None, connected_path=True, path_centroid_aggregate='centroid'):
    assert hasattr(search_path, 'shape'), 'Visualization of path needs a shape information for accessing dimension sizes'
    assert search_path.shape[-1] == 2 or search_path.shape[-1] == 3, 'We expect the last dimension of the search path to match the dimensions of the points. Currently only 2d and 3d is supported'
