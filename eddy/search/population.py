...
class CMAESSearch(EvolutionarySearch):
    ...
    def start(self, objective: Objective):
        ...
        self._covariance = np.eye(self.num_dimensions) * np.mean(np.abs(np.subtract(self._lower, self._upper)))
        super().start(objective)
...
class SpeciesCMAESSearch(PopulationSearch):
    ...
    def sample_random(self, species=0):
        return np.random.multivariate_normal(self._centroids[species], self._covariances[species])
    ...
    def start(self, objective: Objective):
        ...
        self._covariances = [np.eye(self.num_dimensions) * np.mean(np.abs(np.subtract(self._lower, self._upper)))]
    ...
    def step(self):
        ...
        for species, population in enumerate(self._populations):
            ...
            self._centroids[species] = self._centroids[species] + self._learning_rate * (np.mean(member_points, axis=0) - self._centroids[species])
            ...
            for d in range(self.num_dimensions):
                if len(self._centroid_histories[species]) > 2:
                    print('Centroid path for dimension %s is %.4f -> %.4f -> %.4f' % (d, self._centroid_histories[species][-3][d], self._centroid_histories[species][-2][d], self._centroid_histories[species][-1][d]))
                    d0 = self._centroid_histories[species][-3][d]-self._centroid_histories[species][-2][d]
                    d1 = self._centroid_histories[species][-2][d] - self._centroid_histories[species][-1][d]

                    if np.sign(d0) != np.sign(d1):
                        print('Centroid path for dim %s changed direction' % d)
                        print('abs(c[-2]-c[-1]) = %.4f -> %.2f of variance V_d[t-1]=%.4f' % (abs(d0), abs(d0)/self._covariance_histories[species][-2][d,d], self._covariance_histories[species][-2][d,d]))
                        print('abs(c[-1]-c[0]) = %.4f -> %.2f of variance V_d[t]=%.4f' % (abs(d1), abs(d1)/self._covariances[species][d,d], self._covariance_histories[species][-1][d,d]))

                        if self._covariances[species][d,d] > np.maximum(1, self._covariance_histories[species][-2][d,d]):
            ...
            if len(split_dimensions) > 0:
                print('Performing population split along dimensions %s' % split_dimensions)
                centroid_0 = self._centroids[species]
                centroid_1 = np.copy(self._centroid_histories[species][-2])
                centroid_1[split_dimensions] = self._centroids[species][split_dimensions]
                covariance = np.copy(self._covariances[species])
                ...
                self._populations.append({self._encode_member(self.sample_random(new_species)) for _ in range(self._population_size)})

                # delete historical record
                self._centroid_histories[species] = self._centroid_histories[species][-2:]
                self._covariance_histories[species] = self._covariance_histories[species][-2:]
