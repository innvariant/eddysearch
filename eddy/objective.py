...
class GenericObjective(Objective):
    def __init__(self,
        ...
        self._visualization_bounds = visualization_bounds if visualization_bounds is not None else search_bounds
    ...
    def _call(self, x: np.ndarray, *args, **kwargs):
        assert x.shape[0] is self._n_dim, 'Objective was defined with dim=%s but was called with shape %s' % (self._n_dim, x.shape)
        return self._fn(x)
...
RosenbrockObjective = lambda ndim=2: GenericObjective(
    rosenbrock1d,
    np.array([[-2, 2], [-2, 2]]),
    np.array([[1]*ndim]),
    n_dim=ndim,
...
GoldsteinPriceObjective = lambda: GenericObjective(
    goldstein_price1d,
    np.array([[-2, 2], [-2, 2]]),
    minima=np.array([[0, -1]]),
    n_dim=2,
...
LeviN13Objective = lambda: GenericObjective(
    levi_n13_1d,
    np.array([[-8, 8], [-8, 8]]),
    minima=np.array([[1, 1]]),
    n_dim=2,
...
HimmelblauObjective = lambda: GenericObjective(
    himmelblau1d,
    np.array([[-5, 5], [-5, 5]]),
    minima=np.array([[3, 2], [-2.805118, 3.131312], [-3.779310, -3.283186], [3.584428, -1.848126]]),
    n_dim=2,
    color_normalizer=matplotlib.colors.LogNorm()
)
...
CrossInTrayObjective = lambda: GenericObjective(
    crossintray1d,
    np.array([[-5, 5], [-5, 5]]),
...
EggholderObjective = lambda: GenericObjective(
    eggholder1d,
    np.array([[-550, 550], [-420, 420]]),
